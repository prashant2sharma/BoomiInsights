document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateTableAndChart(rows, headers, overallCommentIndex, reasonIndex) {
        ...
        function renderTablePage(page) {
            ...
            rows.slice(startRow, endRow).forEach(row => {
                ...
                if (row[overallCommentIndex] && row[overallCommentIndex].trim() === 'Dev review failed') {
        ...
        const passedRows = rows.filter(row => row[overallCommentIndex] && row[overallCommentIndex].trim() !== 'Dev review failed').length;
        ...
        accuracyMessage.innerHTML = `Total Processes: ${totalRows} <br> Passed: ${passedRows} <br> Failed: ${failedRows} <br> Accuracy: ${accuracy}%`;
    ...
    fileInput.addEventListener('change', (event) => {
        ...
        Papa.parse(file, {
            ...
            complete: function (results) {
                ...
                const overallCommentIndex = headers.indexOf('OverallComment');
    ...
    function validateJsonSchema(json) {
        ...
        json.Insights.forEach((item, index) => {
        ...
        if (totalEntries !== json.Insights.length) {
            throw new Error(`Total Entries (${totalEntries}) does not match the number of insights (${json.Insights.length}).`);
        }
    ...
    submitJson.addEventListener('click', () => {
        ...
        try {
            ...
            const rows = json.Insights.map(item => requiredHeaders.map(header => item[header] || ''));
